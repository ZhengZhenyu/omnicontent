...
**文档版本**: v2.1
**编写日期**: 2026-02-06
**更新日期**: 2026-02-06 (所有ASCII图转换为Mermaid UML)
...
graph TB
    ...
    subgraph Storage["文件存储层 (Storage Layer)"]
        FileSystem["Local File System<br/>/uploads/files/ - DOCX文件<br/>/uploads/images/ - 图片<br/>/uploads/covers/ - 封面图"]
    end
...
```mermaid
graph TB
    subgraph App["单一应用实例 (Single Application Instance)"]
        ...
        ORM["ORM Layer<br/>(自动添加 WHERE community_id = ?)"]

        ORM --> DB

        subgraph DB["Shared Database (共享数据库)"]
            Contents["📊 contents<br/>├─ id<br/>├─ community_id (FK) ← 隔离字段<br/>├─ title<br/>└─ ..."]
            ChannelConfigs["⚙️ channel_configs<br/>├─ id<br/>├─ community_id (FK) ← 隔离字段<br/>├─ channel<br/>└─ ..."]
        end
    end
...
sequenceDiagram
    ...
    Client->>API: 1. HTTP Request<br/>Headers:<br/>• Authorization: Bearer {token}<br/>• X-Community-Id: {id}

    rect rgb(240, 248, 255)
        Note over API: 2. Verify JWT Token
        API->>API: get_current_user()<br/>解析Token获取user_id
        API->>API: 返回 User Object
    end

    rect rgb(255, 245, 240)
        Note over API: 3. Validate Community Access
        API->>API: get_current_community()<br/>检查用户是否属于该社区<br/>(超管可访问所有社区)
        API->>API: 返回 community_id
    end

    rect rgb(240, 255, 240)
        Note over API,DB: 4. Query with Data Isolation
        API->>DB: SELECT * FROM contents<br/>WHERE community_id = ?
        DB-->>API: Filtered Data<br/>(仅返回该社区的数据)
    end

    API-->>Client: 5. JSON Response<br/>(隔离后的数据)
```
...
graph TB
    ...
    Master -->|Replication| Replica[PostgreSQL Replica<br/>只读]

    subgraph Cluster["应用集群 (Application Cluster)"]
        Backend1
        Backend2
        Backend3
    end

    subgraph DBCluster["数据库集群 (Database Cluster)"]
        Master
        Replica
    end
